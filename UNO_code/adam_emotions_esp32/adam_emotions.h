...
//
// Target  : ESP32 + ILI9341 320x240 (landscape)
// Library : Adafruit_GFX + Adafruit_ILI9341
//
// USAGE (in your .ino):
//
...
static AdamEmotion _lastRenderedEmotion = EMO_COUNT;
static Adafruit_GFX* _spr = nullptr;
static Adafruit_ILI9341* _tft = nullptr;
static GFXcanvas16* _canvas = nullptr;
static bool        _spriteReady = false;
static uint8_t     _spriteDepth = 0;

static inline void _clearCanvas(uint16_t color) {
  if (_spr) {
    _spr->fillScreen(color);
  }
}

void initEmotions(Adafruit_ILI9341& tft) {
  _tft = &tft;
  _canvas = new GFXcanvas16(SCR_W, SCR_H);
...
static void _drawHappy(uint32_t ms) {
  ...
  auto _sp = [&](uint32_t offset, int sx, int sy, int arm) {
    ...
    uint16_t col = _rgb565(y, y, (uint8_t)(y * 0.6f));
...
static void _drawSad(uint32_t ms) {
  ...
  auto _tear = [&](float tp, int ex) {
    ...
    uint16_t tc = _rgb565(av/3, av/2, av);
...
static void _drawAngry(uint32_t ms) {
  ...
  uint16_t vc = _rgb565((uint8_t)(200*vb+55), (uint8_t)(60*vb), (uint8_t)(60*vb));
  ...
  if (ht < 960) {
    ...
    uint16_t hc = _rgb565((uint8_t)(160*hb), (uint8_t)(80*hb), 0);
...
static void _drawPanic(uint32_t ms) {
  ...
  uint16_t sc = _rgb565((uint8_t)(200*sb),(uint8_t)(230*sb),(uint8_t)(255*sb));
  ...
  auto _sweat = [&](float phase, int ex) {
    ...
    _spr->fillEllipse(ex, swy, 4, 6, _rgb565(av/4, av/3, av));
  };
...
static void _drawShy(uint32_t ms) {
  ...
  for (int r = 22; r > 0; r -= 4) {
    ...
    uint16_t bc = _rgb565(ri, ri/5, ri/4);
  ...
  if (hf > 0.1f && hf < 2.6f) {
    ...
    _heartOutline(hx, hy, 7, _rgb565(av, av/5, av/4), 2);
  }
}
...
static void _drawLove(uint32_t ms) {
  ...
  if (rb > 0.04f) {
    uint16_t rc = _rgb565((uint8_t)(rb*255), (uint8_t)(rb*80), (uint8_t)(rb*100));
  ...
  auto _fh = [&](float phase, int sx2, int sy) {
    ...
    _heartOutline(hx, hy, hs, _rgb565(av, av/4, av/4), 2);
  };
...
static void _drawRizz(uint32_t ms) {
  ...
  if (phase > 0.50f && phase < 0.80f) {
    ...
    uint16_t gc = _rgb565(ga, ga, (uint8_t)(ga*0.5f)); // warm white
...
void updateEmotion() {
  if (!_spr || !_spriteReady) return;
  uint32_t now = millis();

  // Direct-to-TFT mode on plain ESP32 needs a lower refresh rate to avoid flicker,
  // but still needs periodic redraws so the face can animate.
  if (_spriteDepth == 0) {
    if (_emo != _lastRenderedEmotion) {
      _lastRenderedEmotion = _emo;
      _lastFrame = 0;
    }
    if (now - _lastFrame < 140) return;
  } else {
    if (now - _lastFrame < 50) return;
  }
  ...
  if (_canvas && _tft) {
    _tft->drawRGBBitmap(0, 0, _canvas->getBuffer(), SCR_W, SCR_H);
  }
}

bool emotionsReady() {
  return _spr && _spriteReady;
}

uint8_t emotionSpriteDepth() {
  return _spriteDepth;
}
