"""
ADAM — Autonomous Desktop AI Module (v27)
==========================================
Works with the single system_prompt.txt (compact, token-optimised).
No other prompt files needed.

CHANGES FROM v26:
  • Phase 1 — Detection speedup: Haar cascade runs on 320×240 downscale with
    CLAHE (replaces global equaliseHist). scaleFactor 1.1→1.2 (2× fewer pyramid
    levels). Coordinates scaled back to full-res. 4-5× faster on Pi Zero 2W.
  • Phase 2 — KCF tracker fallback: when cascade misses frames (blink, tilt,
    motion) KCF trackers maintain face bboxes for up to KCF_MAX_MISSES frames.
    Face IDs no longer vanish mid-conversation. Falls back to frozen-position
    tracker if KCF/CSRT/MOSSE are unavailable (no contrib needed).
  • Phase 3 — Gaze detection: _eye_symmetry_score() measures brightness
    symmetry in the eye-strip of each face. facing_camera: bool added to every
    face dict. Adam only activates (ATTENTIVE) when someone is actually looking
    at the camera, not just present in frame.
  • Phase 4 — Optical-flow lip speaker detection: _lip_motion_score() replaces
    the fragile brightness-delta mouth-ROI method. Uses calcOpticalFlowPyrLK on
    goodFeaturesToTrack anchor points in the lip region. Score = mean vertical
    displacement — robust to lighting changes, shadow, and bbox jitter.
  • Phase 5 — webrtcvad VAD: VoiceActivityDetector wraps webrtcvad.Vad (mode 2).
    Fed every mic chunk in listen(). Rolling 10-frame majority vote. Speaker
    confirmed only when optical-flow score AND VAD agree. Gracefully disabled if
    webrtcvad is not installed (falls back to permissive True).
  • Phase 6 — Who is facing whom: facing_ratio (fw/fh) + eye-symmetry score →
    estimated gaze direction (→CAM, →LEFT, →RIGHT) per person. build_context()
    now tells Gemini exactly who is looking at Adam vs talking to each other.
  • Phase 7 — Attention gating: attention.activate() fires only when at least
    one person has facing_camera=True. Neck auto-pan only tracks speakers who
    are facing the camera (ignores side conversations).
  • Banner and SETUP updated for v27.
  • New optional dependency: webrtcvad  (pip install webrtcvad)

SETUP:
    pip install --upgrade google-genai pyaudio python-dotenv websockets flask
                           opencv-python pyperclip pyserial

    # webrtcvad (Phase 5, recommended on Pi Zero 2W — ARMv8-compatible):
...
def _draw_preview(data: dict) -> np.ndarray:
    ...
    cv2.putText(frame, "ADAM v27  [Q=close]", (8, 20),
...
def main_entry() -> None:
    ...
    print("=" * 66)
    print("  ADAM — Autonomous Desktop AI Module  (v27)")
    ...
    print("=" * 66)
    print()
    print("  HOW TO USE (v27 hybrid gaze+speaker tracking):")
