...
CAPABILITIES:
- Google Search is disabled for this test — do NOT request search.
...
- Never say you can't access the internet — you can, via Google Search.
...
def handle_tool_call(tool_call) -> list[dict]:
    ...
    Covers:
      ...
      - google_search         (since March 2026, Live API sends this as a
                               function_call instead of handling server-side)
    """
    responses = []
    ...
    for fc in tool_call.function_calls:
        ...
        if name == "get_current_datetime":
            ...
            # Since Mar 2026 the Live API routes google_search to the client.
            # We acknowledge it so Gemini can continue — actual search results
            # come back from Google's servers after we send this response.
            query = fc.args.get("q", fc.args.get("query", ""))
            print(f"  🔍  [tool] google_search → \"{query}\"")
            # Return empty result; Gemini will merge with its own search data
...
async def run_session(
    ...
    # 2. Tools: only local datetime (Google Search disabled for this test)
    tools = [
