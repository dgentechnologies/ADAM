"""
ADAM — Autonomous Desktop AI Module (v30 test)
==========================================
Works with the single system_prompt.txt (compact, token-optimised).
No other prompt files needed.

CHANGES FROM v29:
    • Added non-blocking DDG test flow:
        - web_search runs in background (no conversational halt)
        - ADAM asks consent when result is ready
        - ADAM speaks only a short summary after user says yes
        - no Google grounding used

CHANGES FROM v28:
  • SEARCH COST OPTIMISATION — fixes the grounding-in-a-loop bug and
    dramatically reduces billable Google Search queries:

    1. DynamicRetrievalConfig added to native GoogleSearch tool:
         dynamic_threshold=0.75 — grounding only fires when the model's
         own confidence score falls below 0.25, i.e. it truly cannot answer
         from training data.  Default threshold was 0.3 (fires far too often).
    2. Removed duplicate camera-context injection in receive():
         The receive() task was re-injecting [LIVE CAMERA CONTEXT: ...] on
         every input_transcription event, creating an extra model "turn" on
         top of what send() already injects at voice onset.  Each extra turn
         is a potential grounding trigger.  Removed the duplicate.
    3. Re-added explicit web_search(query) FunctionDeclaration backed by
         DuckDuckGo (duckduckgo-search package, free, no API key):
         - ADAM explicitly calls this for most look-up queries.
         - 30-minute result cache prevents re-querying the same topic.
         - 5-second rate-limit gate prevents burst calls.
         - Native Google grounding (expensive) is now the last resort only
           for queries marked as needing live/breaking-news data.
    4. System-prompt search rules tightened:
         web_search = free DDG (use first), google grounding = live breaking
         news only (use sparingly).
  • SETUP updated for v29 (duckduckgo-search optional but recommended).
  • Banner and HOW-TO-USE updated for v29.
...
SETUP:
    ...
    pip install vosk
    pip install duckduckgo-search      # free DDG search — strongly recommended
    (Download vosk model: https://alphacephei.com/vosk/models)

RUN:
    python adamV30_test_async_search.py
"""
...
# Physical neck tracking (smoothed, pan+tilt)
NECK_PAN_CENTER       = 90
NECK_TILT_CENTER      = 85
NECK_PAN_MIN          = 30
NECK_PAN_MAX          = 150
NECK_TILT_MIN         = 50
NECK_TILT_MAX         = 120
NECK_PAN_DEADZONE     = 10         # degrees from target before pan updates
NECK_TILT_DEADZONE    = 6          # degrees from target before tilt updates
NECK_TRACK_INTERVAL   = 0.12       # seconds between tracking updates
NECK_MAX_STEP         = 2          # max degrees moved per update (smoother motion)
NECK_RECENTER_HOLD_S  = 1.5        # recenter delay when no valid face target
NECK_FACE_TRACK_ALPHA_PAN = 0.32   # pan target smoothing factor
NECK_FACE_TRACK_ALPHA_TILT = 0.18  # tilt target smoothing factor (lower to reduce up/down jitter)
NECK_IDLE_START_AFTER_S = 4.0      # start autonomous idle sway after no face for this long
NECK_IDLE_SWAY_PAN_AMPL = 10.0     # idle pan amplitude in degrees
NECK_IDLE_SWAY_TILT_AMPL = 4.0     # idle tilt amplitude in degrees
NECK_IDLE_SWAY_PERIOD_S = 6.0      # full idle sway period
NECK_SPEECH_BOB_MAX_DEG = 3.5      # max additional tilt during speech micro-bob
...
# Session-scoped DDG search cache: query → (result_text, fetched_timestamp)
_ddg_cache:   dict[str, tuple[str, float]] = {}
_last_ddg_t:  float                        = 0.0

SEARCH_READY_PROMPT = (
    "Result is ready. Ask the user: do you want to know the short summary now?"
)


def detect_language(text: str) -> str:
    """Lightweight language guess for dynamic search prompts."""
    ...
    # Lexical fallback
    if re.search(r"\b(que|como|donde|resultado|noticias|hola|gracias)\b", t):
        return "es"
    if re.search(r"\b(bonjour|merci|resultat|nouvelles|comment|quoi)\b", t):
        return "fr"
    if re.search(r"\b(hallo|danke|ergebnis|nachrichten|wie|was)\b", t):
        return "de"
    if re.search(r"\b(ciao|grazie|risultato|notizie|come|cosa)\b", t):
        return "it"
...
def detect_query_topic(query: str) -> str:
    """Rough topic classifier for more situational search lines."""
    q = (query or "").strip().lower()
    if not q:
        return "general"
    if re.search(r"\b(weather|temperature|rain|storm|forecast|climate|humidity)\b", q):
        return "weather"
    if re.search(r"\b(election|vote|result|breaking|news|headline|politics|minister)\b", q):
        return "news"
    if re.search(r"\b(match|score|ipl|football|cricket|nba|fifa|goal|wicket)\b", q):
        return "sports"
    if re.search(r"\b(price|stock|market|gold|crypto|bitcoin|rupee|usd|rate)\b", q):
        return "price"
    return "general"


def _pick_topic_suffix(lang: str, topic: str) -> str:
    suffix_map = {
        "en": {
            "weather": "I will pull fresh weather signals.",
            "news": "I will verify the latest updates from live sources.",
            "sports": "I will check live sports feeds for this one.",
            "price": "I will verify the latest numbers before I answer.",
            "general": "I will cross-check it quickly on live sources.",
        },
        "bn": {
            "weather": "লাইভ আবহাওয়ার আপডেট দেখে নিচ্ছি।",
            "news": "সর্বশেষ খবর লাইভ সোর্স থেকে যাচাই করছি।",
            "sports": "লাইভ স্পোর্টস আপডেট দেখে নিচ্ছি।",
            "price": "সর্বশেষ দামের তথ্য যাচাই করছি।",
            "general": "লাইভ সোর্স থেকে দ্রুত যাচাই করছি।",
        },
        "hi": {
            "weather": "मैं ताज़ा मौसम अपडेट चेक कर रहा हूं।",
            "news": "मैं लाइव स्रोतों से लेटेस्ट अपडेट वेरिफाई कर रहा हूं।",
            "sports": "मैं लाइव स्पोर्ट्स अपडेट अभी चेक करता हूं।",
            "price": "मैं लेटेस्ट रेट्स पहले कन्फर्म कर लेता हूं।",
            "general": "मैं इसे लाइव स्रोतों से जल्दी verify करता हूं।",
        },
    }
    return suffix_map.get(lang, suffix_map["en"]).get(topic, suffix_map.get(lang, suffix_map["en"])["general"])


def dynamic_search_start_line(user_text: str, query: str = "") -> str:
    """Creative line ADAM can say when internet lookup starts."""
    lang = detect_language(user_text)
    topic = detect_query_topic(query)
    options = {
        "bn": [
            "দারুণ প্রশ্ন। একটু সময় দিন, লাইভ ওয়েবে যাচাই করছি।",
            "চলুন, এটা একবার দ্রুত অনলাইনে ক্রস-চেক করি।",
            "অন দ্য জব। এখনই লাইভ সোর্সে ডাইভ দিচ্ছি।",
            "ওকে, এখনই নেট থেকে ফ্যাক্ট-চেক করে আসছি।",
        ],
        "hi": [
            "बढ़िया सवाल, एक सेकंड. मैं अभी लाइव वेब से verify करता हूं।",
            "ठीक है, इसे ऑनलाइन तुरंत cross-check करके बताता हूं।",
            "ऑन इट. मैं अभी नेट से facts lock करता हूं।",
            "एक पल, मैं fresh sources से इसे confirm कर लेता हूं।",
        ],
        "es": [
            "Dame un momento, lo verifico en internet ahora mismo.",
            "Perfecto, voy a contrastarlo en la web y vuelvo enseguida.",
            "Un segundo, reviso fuentes en línea para confirmarlo.",
        ],
        "fr": [
            "Donne-moi une seconde, je verifie ca sur le web.",
            "Parfait, je vais confirmer ca en ligne tout de suite.",
            "Un instant, je consulte des sources web en direct.",
        ],
        "de": [
            "Einen Moment, ich pruefe das kurz im Internet.",
            "Alles klar, ich verifiziere das direkt online.",
            "Kurz warten, ich checke Live-Quellen im Web.",
        ],
        "it": [
            "Dammi un attimo, lo verifico subito online.",
            "Perfetto, controllo al volo sul web e torno.",
            "Un secondo, faccio un check rapido su fonti online.",
        ],
        "en": [
            "Nice one. Give me a beat, I will verify this on live web sources.",
            "On it. Let me run a quick internet sweep.",
            "Hold that thought. I am cross-checking this online now.",
            "Smart ask. I will pull fresh web signals and verify.",
        ],
    }
    base = random.choice(options.get(lang, options["en"]))
    return f"{base} {_pick_topic_suffix(lang, topic)}".strip()


def dynamic_search_ready_line(user_text: str, query: str = "") -> str:
    """Creative follow-up asking whether to share the found summary now."""
    lang = detect_language(user_text)
    topic = detect_query_topic(query)
    options = {
        "bn": [
            "রেজাল্ট রেডি। এখনই ছোট করে বলব?",
            "খুঁজে পেয়েছি। সারাংশটা এখন শুনতে চান?",
            "আপডেট এসে গেছে, এখন বলব নাকি একটু পরে?",
            "যাচাই শেষ। কুইক ভার্সনটা এখন দেব?",
        ],
        "hi": [
            "रिजल्ट तैयार है, अभी छोटा सार बताऊं?",
            "मिल गया. अभी summary सुनना चाहेंगे?",
            "अपडेट आ गया है, अभी बताऊं या बाद में?",
            "कन्फर्म हो गया. quick version अभी सुनना है?",
        ],
        "es": [
            "Ya tengo el resultado. Te doy un resumen corto ahora?",
            "Listo, lo encontre. Quieres que te lo resuma ya?",
            "Tengo la actualizacion. Te la cuento ahora mismo?",
        ],
        "fr": [
            "Resultat pret. Tu veux un resume rapide maintenant ?",
            "C'est trouve. Je te fais un court resume tout de suite ?",
            "J'ai l'update. Je te la donne maintenant ?",
        ],
        "de": [
            "Ergebnis ist da. Soll ich dir jetzt eine kurze Zusammenfassung geben?",
            "Gefunden. Willst du die Kurzfassung sofort?",
            "Update ist bereit. Soll ich es jetzt kurz erklaeren?",
        ],
        "it": [
            "Risultato pronto. Vuoi un riassunto rapido adesso?",
            "Trovato. Te lo riassumo subito?",
            "Ho l'aggiornamento. Vuoi che te lo dica ora?",
        ],
        "en": [
            "Result is ready. Want the short summary now?",
            "I found it. Should I give you the quick version?",
            "Update is in. Want me to break it down now?",
            "Verification complete. Want the fast brief now?",
        ],
    }
    lead = random.choice(options.get(lang, options["en"]))
    topic_tag = {
        "en": {
            "weather": "I can also include a short forecast angle.",
            "news": "I can include the key headline and source angle.",
            "sports": "I can include the latest scoreline context too.",
            "price": "I can include the latest value snapshot as well.",
            "general": "I can keep it to two crisp lines.",
        },
        "bn": {
            "weather": "চাইলে ছোট্ট ফোরকাস্টও যোগ করব।",
            "news": "চাইলে মূল হেডলাইন আর সোর্স অ্যাঙ্গেলও বলব।",
            "sports": "চাইলে লেটেস্ট স্কোর কনটেক্সটও যোগ করব।",
            "price": "চাইলে সর্বশেষ ভ্যালু স্ন্যাপশটও দেব।",
            "general": "চাইলে দুই লাইনে একদম ক্রিস্প করে বলব।",
        },
        "hi": {
            "weather": "चाहो तो छोटा forecast angle भी जोड़ दूंगा।",
            "news": "चाहो तो key headline और source angle भी दूंगा।",
            "sports": "चाहो तो latest score context भी जोड़ दूंगा।",
            "price": "चाहो तो latest value snapshot भी दे दूंगा।",
            "general": "चाहो तो दो crisp lines में बताता हूं।",
        },
    }
    addon = topic_tag.get(lang, topic_tag["en"]).get(topic, topic_tag.get(lang, topic_tag["en"])["general"])
    return f"{lead} {addon}".strip()
...
def load_system_prompt(memory: dict, faces: dict,
    ...
    search_test_rules = (
        "━━━ SEARCH TEST MODE (v30) ━━━\n"
        "- For internet lookups, call web_search(query) once.\n"
        "- Immediately tell the user: 'Wait, let me check the internet.'\n"
        "- web_search runs in background; continue normal conversation while waiting.\n"
        "- When system says result is ready, ask: 'Do you want to know?'\n"
        "- Only if user confirms, provide a short summary in 1-2 sentences.\n"
        "- If user says no, discard the result and continue.\n"
        "- Never dump full raw search snippets to the user."
    )
...
# ═════════════════════════════════════════════════════════════════════════════
# WEB SEARCH  — DuckDuckGo, cached, rate-limited  (v29)
# ═════════════════════════════════════════════════════════════════════════════

async def web_search(query: str, max_results: int = 5) -> str:
    """
    Perform a DuckDuckGo search for *query* and return a compact text summary.

    Optimisation layers:
      1. Result cache (SEARCH_CACHE_TTL_S=1800s) — identical queries served
         from cache, zero network cost.
      2. Rate gate (SEARCH_MIN_GAP_S=5s) — prevents burst calls within one
         conversational turn.
      3. Falls back to a clear 'not available' message so ADAM can still
    ...
    # ── Cache hit ─────────────────────────────────────────────────────────────
    if q in _ddg_cache:
        cached_text, cached_t = _ddg_cache[q]
        if now - cached_t < SEARCH_CACHE_TTL_S:
            print(f"  🔍  [DDG cache hit] \"{query}\"")
            return cached_text
    ...
    try:
        def _run_ddg_search():
            # Fallback compatibility: silence one-time rename warning emitted
            # by duckduckgo_search in newer environments.
            if DDG_BACKEND == "duckduckgo_search":
    ...
    # Build a compact summary: title + snippet per result (no raw URLs)
    lines: list[str] = []
    for r in results:
    ...
    summary = "\n".join(lines)
    _ddg_cache[q] = (summary, time.time())
    return summary


def shorten_search_summary(raw_text: str, max_lines: int = 3,
                           max_chars_per_line: int = 160) -> str:
    """Compress DDG snippets into a short, speech-friendly summary."""
    cleaned = [ln.strip(" •\t") for ln in raw_text.splitlines() if ln.strip()]
    if not cleaned:
        return "I could not find a solid result."

    out: list[str] = []
    for ln in cleaned[:max_lines]:
        compact = " ".join(ln.split())
        if len(compact) > max_chars_per_line:
            compact = compact[: max_chars_per_line - 3].rstrip() + "..."
        out.append(compact)
...
def is_affirmative(text: str) -> bool:
    """Return True for common affirmative replies."""
    t = text.strip().lower()
    if not t:
        return False
    return bool(re.search(r"\b(yes|yeah|yep|sure|ok|okay|haan|ha|bolo|tell me|go ahead)\b", t))


def is_negative(text: str) -> bool:
    """Return True for common negative replies."""
    t = text.strip().lower()
    if not t:
        return False
    return bool(re.search(r"\b(no|nope|nah|not now|later|cancel|skip)\b", t))
...
def _draw_preview(data: dict) -> np.ndarray:
    ...
    cv2.putText(frame, "ADAM v30 test  [Q=close]", (8, 20),
...
async def handle_tool_call(tc, memory: dict, faces: dict,
                           current_frame=None,
                           queue_web_search=None,
                           get_last_user_text=None) -> list[dict]:
    responses = []
    for fc in tc.function_calls:
        ...
        # ── Story / event memory ──────────────────────────────────────────────
            ...
            if not query:
                result = {"status": "error", "reason": "query cannot be empty"}
            elif queue_web_search is None:
                result = {"status": "error", "reason": "search queue unavailable"}
            else:
                await queue_web_search(query)
                print(f"  🌐  DDG queued in background: \"{query}\"")
                user_text = get_last_user_text() if callable(get_last_user_text) else ""
                result = {
                    "status": "queued",
                    "query": query,
                    "confirmation": dynamic_search_start_line(user_text, query),
                }
...
def build_tools() -> list[types.Tool]:
    ...
    fn_tool = types.Tool(function_declarations=[
        ...
        # ── v29: explicit DDG search — free, cached, rate-limited ──────────────────
        types.FunctionDeclaration(
            name="web_search",
            description=(
                "Search the internet via DuckDuckGo for factual look-ups, definitions, "
                "news, prices, weather, sports scores, or any subject you aren’t certain "
                "about.  Use this FIRST for any search need.  Results are cached for "
                "30 minutes, so calling the same query twice is free.  "
                "Do NOT call for: people in PEOPLE YOU KNOW, DGEN facts, identity "
                "questions, or anything you already know from training data."
            ),
            parameters=S(type=T.OBJECT, properties={
...
async def run_session(
    ...
    try:
        async with client.aio.live.connect(model=LIVE_MODEL, config=config) as session:
            ...
            search_jobs_q    = asyncio.Queue(maxsize=20)
            search_done_q    = asyncio.Queue(maxsize=20)
            queued_searches: set[str] = set()
            search_state     = {
            ...
            async def safe_send_system_text(text: str,
                                            retries: int = 24,
                                            retry_delay_s: float = 0.25) -> bool:
                """Best-effort system text injection with retry for transient live-state conflicts."""
                ...
                if last_err is not None:
                    print(f"  ⚠️  System text inject failed after retries: {last_err}")
                return False

            async def queue_web_search(query: str) -> None:
                """Queue a DDG query for background execution."""
                q = query.strip()
                if not q:
                    return
                if q in queued_searches:
                    print(f"  🌐  DDG already queued: \"{q}\"")
                    return
                try:
                    search_jobs_q.put_nowait(q)
                    queued_searches.add(q)
                except asyncio.QueueFull:
                    _ = await search_jobs_q.get()
                    search_jobs_q.put_nowait(q)
                    queued_searches.add(q)

            async def search_worker() -> None:
                """Run DDG lookups in background without blocking live interaction."""
                try:
                    while not stop.is_set():
                        query = await search_jobs_q.get()
                        try:
                            raw = await web_search(query)
                            short = shorten_search_summary(raw)
                            await search_done_q.put({
                                "query": query,
                                "summary": short,
                            })
                        except Exception as e:
                            ...
                            queued_searches.discard(query)
                except asyncio.CancelledError:
                    pass

            async def notify_search_ready() -> None:
                """Notify model when background search finishes so it can ask user consent."""
                try:
                    while not stop.is_set():
                        done = await search_done_q.get()
                        search_state["awaiting_user_confirmation"] = True
                        search_state["summary"] = done.get("summary", "")
                        search_state["query"] = done.get("query", "")
                        print(f"  🌐  DDG result ready for: \"{search_state['query']}\"")
                        dynamic_ready = dynamic_search_ready_line(
                            last_user_utterance[0],
                            search_state["query"],
                        )
                        sent = await safe_send_system_text(
                            text=(
                                f"[SYSTEM: Search completed for query: {search_state['query']}. "
                                "Reply in the same language as the user and ask naturally if they "
                                "want the short result now. Keep it creative and concise. "
                                f"Suggested style: {dynamic_ready}]"
                            )
                        )
                        if not sent:
                            print("  ⚠️  Could not notify model about ready search")
                except asyncio.CancelledError:
                    pass
            ...
            async def camera() -> None:
                ...
                curr_pan     = NECK_PAN_CENTER
                curr_tilt    = NECK_TILT_CENTER
                aim_pan      = float(NECK_PAN_CENTER)
                aim_tilt     = float(NECK_TILT_CENTER)
                idle_phase   = random.uniform(0.0, 2.0 * np.pi)
                ...
                try:
                    ...
                    while not stop.is_set():
                        ...
                        # ── Phase 7: Neck auto-track (pan+tilt), also while speaking ─
                        now = time.time()
                        if (neck_is_ready()
                                and now - last_track_t >= NECK_TRACK_INTERVAL):
                            ...
                            if (spk_idx is not None
                                ...
                                target_face = faces_[spk_idx]
                            elif faces_:
                                facing_faces = [f for f in faces_ if f["facing_camera"]]
                                target_face = facing_faces[0] if facing_faces else faces_[0]

                            if target_face is not None:
                                last_face_t = now
                                cx = target_face["cx_norm"]
                                cy = target_face["cy_norm"]

                                # Camera preview is mirrored for pan axis.
                                raw_target_pan = float(NECK_PAN_MAX - cx * (NECK_PAN_MAX - NECK_PAN_MIN))
                                raw_target_tilt = float(NECK_TILT_MIN + cy * (NECK_TILT_MAX - NECK_TILT_MIN))
                                aim_pan += (raw_target_pan - aim_pan) * NECK_FACE_TRACK_ALPHA_PAN
                                aim_tilt += (raw_target_tilt - aim_tilt) * NECK_FACE_TRACK_ALPHA_TILT
                            elif now - last_face_t >= NECK_IDLE_START_AFTER_S:
                                if attention.state == AttentionState.PASSIVE:
                                    t = (now / NECK_IDLE_SWAY_PERIOD_S) * (2.0 * np.pi) + idle_phase
                                    aim_pan = float(NECK_PAN_CENTER) + NECK_IDLE_SWAY_PAN_AMPL * np.sin(t)
                                    # Slightly slower tilt sway for natural motion
                                    aim_tilt = float(NECK_TILT_CENTER) + NECK_IDLE_SWAY_TILT_AMPL * np.sin(t * 0.6)
                                elif now - last_face_t >= NECK_RECENTER_HOLD_S:
                                    aim_pan += (float(NECK_PAN_CENTER) - aim_pan) * 0.20
                                    aim_tilt += (float(NECK_TILT_CENTER) - aim_tilt) * 0.20
                            else:
                                # Short no-face gap: keep current aim to avoid twitching.
                                aim_pan = float(curr_pan)
                                aim_tilt = float(curr_tilt)

                            target_pan = int(max(NECK_PAN_MIN, min(NECK_PAN_MAX, aim_pan)))
                            target_tilt = int(max(NECK_TILT_MIN, min(NECK_TILT_MAX, aim_tilt)))

                            # Speech-synced micro motion overlay: keeps neck movement
                            # parallel with audio playback instead of serially before/after speech.
                            if adam_speaking.is_set():
                                age = now - float(speech_motion["last_update"])
                                level = float(speech_motion["level"])
                                if age > 0.20:
                                    # Decay stale level if no recent audio chunk update.
                                    level *= 0.82
                                    speech_motion["level"] = level
                                bob = max(0.0, np.sin(float(speech_motion["phase"])))
                                bob_deg = min(NECK_SPEECH_BOB_MAX_DEG, (0.8 + level * NECK_SPEECH_BOB_MAX_DEG) * bob)
                                target_tilt = int(max(NECK_TILT_MIN, min(NECK_TILT_MAX, target_tilt + bob_deg)))

                            if abs(target_pan - curr_pan) >= NECK_PAN_DEADZONE:
                                next_pan = _step_towards(curr_pan, target_pan, NECK_MAX_STEP)
                                if next_pan != curr_pan:
                                    await asyncio.to_thread(pan, next_pan)
                                    curr_pan = next_pan

                            if abs(target_tilt - curr_tilt) >= NECK_TILT_DEADZONE:
                                next_tilt = _step_towards(curr_tilt, target_tilt, NECK_MAX_STEP)
                                if next_tilt != curr_tilt:
                                    await asyncio.to_thread(tilt, next_tilt)
                                    curr_tilt = next_tilt
                        ...
                        async for msg in session.receive():
                            ...
                            if msg.tool_call:
                                resps = await handle_tool_call(
                                    ...
                                    latest_frame[0], queue_web_search,
                                    lambda: last_user_utterance[0],
                                )
                            ...
                            if sc.input_transcription and sc.input_transcription.text:
                                ...
                                last_user_utterance[0] = transcript

                                if search_state["awaiting_user_confirmation"]:
                                    if is_affirmative(transcript):
                                        summary = search_state["summary"]
                                        q_text = search_state["query"]
                                        search_state["awaiting_user_confirmation"] = False
                                        search_state["summary"] = ""
                                        search_state["query"] = ""
                                        sent = await safe_send_system_text(
                                            text=(
                                                "[SYSTEM: User confirmed they want the search result. "
                                                f"Query: {q_text}. "
                                                f"Deliver ONLY this short summary in max 2 sentences: {summary}]"
                                            )
                                        )
                                        if not sent:
                                            print("  ⚠️  Could not inject confirmed summary")
                                        ...
                                        sent = await safe_send_system_text(
                                            text=(
                                                "[SYSTEM: User declined the pending search result. "
                                                "Acknowledge briefly and continue naturally.]"
                                            )
                                        )
                                        if not sent:
                                            print("  ⚠️  Could not inject decline notice")
                                ...
                                # v29: duplicate [LIVE CAMERA CONTEXT] injection removed.
                                # send() injects context at voice onset via ctx_injected
                                # gate. Re-injecting here on every transcription event
                                # created extra model turns that looped grounding calls.
                            ...
                            if sc.model_turn:
                                ...
                                for part in sc.model_turn.parts:
                                    ...
                                    # ── Native grounding observability (v29) ──
                ...
                async def end_of_turn() -> None:
                    ...
                    while not mic_q.empty():
                        try:
                            ...
                            break
                    adam_speaking.clear()
                    speech_motion["level"] = 0.0
                    speech_motion["phase"] = 0.0
                    speech_motion["last_update"] = time.time()
                    ...
                    while not stop.is_set():
                        try:
                            ...
                            if chunk is None:
                                ...
                                # Estimate speech energy from the exact chunk being played,
                                # so neck micro-motion tracks voice in real time.
                                try:
                                    s = np.frombuffer(chunk, dtype=np.int16)
                                    if s.size > 0:
                                        rms = float(np.sqrt(np.mean(np.square(s.astype(np.float32)))))
                                        norm = min(1.0, rms / 9000.0)
                                        speech_motion["level"] = (speech_motion["level"] * 0.65) + (norm * 0.35)
                                        speech_motion["phase"] = (speech_motion["phase"] + 0.45 + speech_motion["level"] * 0.75) % (2.0 * np.pi)
                                        speech_motion["last_update"] = time.time()
                                except Exception:
                                    pass
            ...
            t_sw  = asyncio.create_task(search_worker(), name="search_worker")
            t_sn  = asyncio.create_task(notify_search_ready(), name="search_notifier")
            ...
            t_sw.cancel()
            t_sn.cancel()
            await asyncio.gather(t_cam, t_l, t_s, t_r, t_p, t_i, t_sw, t_sn,
...
def main_entry() -> None:
    ...
    print("=" * 66)
    print("  ADAM — Autonomous Desktop AI Module  (v30 test)")
    ...
    _ddg_status = "✅ ready — free web search + cache active" if DDG_AVAILABLE else "⚠️  not installed (pip install ddgs)"
    print(f"  DDG search  : {_ddg_status}")
    ...
    print("=" * 66)
    print()
    print("  HOW TO USE (v30 test — background DDG + consent gate):")
    ...
    print("  ⑦ Ask for internet info      →  ADAM says 'wait, checking internet',")
    print("                                   keeps chatting, then asks before brief summary")
    if neck_ok:
