"""
ADAM — Autonomous Desktop AI Module (v24)
==========================================
Works with the new single system_prompt.txt (compact, token-optimised).
No other prompt files needed — delete prompt_vision.txt, prompt_search.txt,
prompt_attention.txt, prompt_clipboard.txt, prompt_language.txt, gen_system_prompt.txt.

CHANGES FROM v23:
  • load_system_prompt() no longer builds an inline vision_block — the new
    system_prompt.txt already contains all personality, vision, search and
    memory rules. Duplication removed → cleaner context, less hallucination.
  • send() ctx_injected flag now resets correctly on attention loss (not just
    when ADAM starts speaking), so every new voice-onset always gets a fresh
    camera snapshot + speaker-context injection.
  • Removed unused `import sys`.
  • run_preview_thread(): Q-key check runs even when no new frame arrived,
    preventing a frozen window when the camera isn't producing frames.
  • main_entry() banner updated to v24.

SETUP:
    pip install --upgrade google-genai pyaudio python-dotenv websockets flask
                           opencv-python pyperclip
...
# Detects faces, tracks mouth movement to estimate who is speaking.
# Gesture detection removed (was unreliable).
# ═════════════════════════════════════════════════════════════════════════════
...
class PersonTracker:
    ...
    def process_frame(self, frame: np.ndarray) -> dict:
        ...
        faces = sorted(raw, key=lambda f: f[0])   # left → right ordering
        result["face_count"] = len(faces)

        # Clean stale mouth history for faces no longer in frame
        ...
        # Update active speaker (frozen while ADAM is speaking)
    ...
    # ── Build a compact text description for prompt injection ─────────────────
    def build_context(self, tr: dict) -> str:
        ...
        if count == 1:
            ...
            return base + (", mouth moving — likely speaking.]" if spk == 0
                           else ".]")
...
# ═════════════════════════════════════════════════════════════════════════════
# PERSISTENT MEMORY — load / save helpers
# ═════════════════════════════════════════════════════════════════════════════
...
# Reads system_prompt.txt (the single compact file) and prepends the dynamic
# memory + faces blocks. Nothing else is injected — the file contains all rules.
# ═════════════════════════════════════════════════════════════════════════════
...
def load_system_prompt(memory: dict, faces: dict) -> str:
    """
    Final prompt = [memory block] + [faces block] + [system_prompt.txt content]
    The file contains all personality, vision, tool-use, language rules.
    No inline vision_block or extra sections added here.
    """
    ...
    # Strip triple-quote wrapper if present (legacy format)
    if base.startswith('"""') and base.endswith('"""'):
        base = base[3:-3].strip()

    parts = [p for p in [_build_memory_block(memory),
...
# ═════════════════════════════════════════════════════════════════════════════
# GEN CLIENT — clipboard generation (secondary model)
# ═════════════════════════════════════════════════════════════════════════════
...
_last_sync_t   = 0.0
_SYNC_INTERVAL = 0.06   # seconds between mouth-sync WebSocket messages
...
# ═════════════════════════════════════════════════════════════════════════════
# CAMERA PREVIEW WINDOW  (debug — runs in its own daemon thread)
# ═════════════════════════════════════════════════════════════════════════════
...
def _draw_preview(data: dict) -> np.ndarray:
    ...
    for i, f in enumerate(tr.get("faces", [])):
        ...
        # Mouth ROI indicator (cyan on speaker, blue on others)
...
def run_preview_thread():
    ...
    while not _preview_stop.is_set():
        # Always poll for Q-key, even when no new frame is queued
        if window_open:
...
async def handle_tool_call(tc, memory: dict, faces: dict,
    ...
    for fc in tc.function_calls:
        ...
        # ── Google Search ─────────────────────────────────────────────────────
            ...
            # Return empty acknowledgement; Gemini merges server-side results
...
def build_tools() -> list[types.Tool]:
    ...
    fn_tool = types.Tool(function_declarations=[
        ...
        types.FunctionDeclaration(
            name="set_emotion",
            description=(
                "Display an emotion on ADAM's OLED face. Call frequently to mirror the "
                "user's emotional state or express ADAM's own reaction."
            ),
            parameters=S(type=T.OBJECT, properties={
...
async def run_session(
    ...
    try:
        async with client.aio.live.connect(model=LIVE_MODEL, config=config) as session:
            ...
            async def camera():
                ...
                try:
                    ...
                    while not stop.is_set():
                        await asyncio.sleep(0.15)   # ~6 FPS tracking loop
                        ...
                        # Run tracker (thread so OpenCV doesn't block the event loop)
                        ...
                        if (now - last_sent >= CAMERA_FPS_INTERVAL
                            ...
                            # Draw lightweight speaker labels so Gemini sees them
                            frame_to_send = raw
                            if tr["face_count"] > 1:
                                ann = raw.copy()
                                spk = tr["active_speaker_idx"]
                        ...
                        # Push to preview window (non-blocking, drops stale frame)
                        if SHOW_PREVIEW:
                            pdata = {
            ...
            # ═══════════════════════════════════════════════════════════════
            # SEND TASK — audio gate + camera context injection on voice onset
            # ═══════════════════════════════════════════════════════════════
            async def send():
                # ctx_injected: True once we've sent the camera snapshot for
                # this voice turn. Reset on ADAM-speaking OR attention loss so
                # every new voice onset always gets a fresh context injection.
                ctx_injected = False
                try:
                    while not stop.is_set():
                        chunk = await mic_q.get()

                        # Gate while ADAM is speaking or not being addressed
                        if adam_speaking.is_set():
                            ctx_injected = False   # reset for next turn
                            continue
                        if not attention.is_active():
                            ctx_injected = False   # reset on attention loss
                            continue

                        # Measure RMS to detect actual speech
                        is_speech = False
                        try:
                            n = len(chunk) // 2
                        ...
                        # Voice onset: inject camera snapshot + who-is-speaking notice
                        if is_speech and not ctx_injected:
                            ...
                            try:
                                ...
                                # 2. Build speaker-context notice
                ...
                try:
                    while not stop.is_set():
                        async for msg in session.receive():
                            ...
                            # Input transcription — fallback wake word + context reinforce
                            if sc.input_transcription and sc.input_transcription.text:
                                transcript = sc.input_transcription.text.strip()
                                print(f"  🗣️  You: {transcript}")

                                # Wake word fallback (if Vosk missed it)
                                ...
                                # Record confirmed speaker for change detection
                                tr  = last_tr[0]
                                spk = tr.get("active_speaker_idx")
                                if spk is not None:
                                    last_spk_idx[0] = spk

                                # Reinforce camera context after audio is finalised
                                ctx = last_camera_ctx[0]
                                if ctx:
                                    try:
                                        await session.send_realtime_input(
                            ...
                            # Turn complete
                            if sc.turn_complete:
                                await out_q.put(None)   # sentinel → speaker drains
            ...
            # ═══════════════════════════════════════════════════════════════
            # SPEAKER TASK — audio output + end-of-turn cleanup
            # ═══════════════════════════════════════════════════════════════
            async def speaker():
                stream = pya.open(
                    format=FORMAT, channels=CHANNELS,
                ...
                WATCHDOG_S    = 1.5   # force-clear if stuck this long

                async def end_of_turn():
                    """Shared cleanup for both sentinel and watchdog paths."""
                    await ws_broadcast({"type": "mouth_sync", "intensity": "closed"})
                    await asyncio.sleep(0.10)
                    await asyncio.sleep(POST_SPEECH_MUTE_S)
                    # Drain any late-arriving audio chunks
                    drained = 0
                    ...
                    # Flush stale mic buffer to prevent echo self-trigger
                ...
                try:
                    while not stop.is_set():
                        try:
                            ...
                            # Watchdog: ADAM was speaking but audio stopped unexpectedly
                            if (adam_speaking.is_set()
            ...
            # ═══════════════════════════════════════════════════════════════
            # IDLE WATCHER — breaks long silences
            # ═══════════════════════════════════════════════════════════════
            ...
            # Session ends when either send or receive finishes
...
async def main():
    ...
    while not stop.is_set():
        if attempt > 0:
            ...
            # Flush stale audio from previous session
            while not out_q.empty():
        ...
        attempt      += 1
        # Rebuild prompt so any memory saved this session appears in the next
        system_prompt = load_system_prompt(memory, faces)
...
def main_entry():
    ...
    print("=" * 66)
    print("  ADAM — Autonomous Desktop AI Module  (v24)")
